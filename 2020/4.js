...
function check() {
	...
	while (i < input.length) {
		...
		if (line == "") {
			if (map["byr"] && map["iyr"] && map["eyr"] && map["hgt"] && map["hcl"] && map["ecl"] && map["pid"]) {
				valid++;
			}
			map = {}
			continue;
...
function check2() {
	...
	while (i < input.length) {
		...
		if (line != "") {
			...
			const {byr, iyr, eyr, hgt, hcl, ecl, pid} = map
			map = {}

			if (!byr || !iyr || !eyr || !hgt || !hcl || !ecl || !pid) continue;

			if (byr.length != 4 || parseInt(byr, 10) < 1920 || parseInt(byr, 10) > 2002) continue;
			if (iyr.length != 4 || parseInt(iyr, 10) < 2010 || parseInt(iyr, 10) > 2020) continue;
			if (eyr.length != 4 || parseInt(eyr, 10) < 2020 || parseInt(eyr, 10) > 2030) continue;

			let match = hgt.match(/(\d+)(cm|in)/)
			if (!match) continue;

			let [, length, unit] = match

			if (unit != 'cm' && unit != 'in') continue;
			if (unit == 'cm' && (parseInt(length, 10) < 150 || parseInt(length, 10) > 193)) continue;
			if (unit == 'in' && (parseInt(length, 10) < 59 || parseInt(length, 10) > 76)) continue;

			if (!hcl.match(/^\#[0-9a-f]{6}$/)) continue;

			var eyes = {"amb": 1, "blu": 1, "brn": 1, "gry": 1, "grn": 1, "hzl": 1, "oth": 1 }

			if (!eyes[ecl]) continue;
